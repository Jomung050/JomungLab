...
class DiaClimatico(RegistroMeteorologico):
    ...
    def descripcion(self):
        ...
        """
        # TU CÓDIGO AQUÍ
        categoria = self.clasificar()
        return f"{self.fecha} | max={self.temp_max:.1f}°C  min={self.temp_min:.1f}°C | {categoria} | Viento: {self.viento_max:.1f} km/h"
...
class RegistroAnual:
    ...
    def temp_promedio_anual(self):
        ...
        suma_maximas = 0
        for dia in self._dias:
            suma_maximas += dia.temp_max
        return round(suma_maximas / len(self._dias), 1)
    ...
    def resumen(self):
        ...
        promedio = self.temp_promedio_anual()

        print(f"=== RESUMEN ANUAL: {self.ciudad} ({self.anio}) ===")
        print(f"Total de días registrados: {len(self)}")
        if caluroso:
            print(f"Día más caluroso: {caluroso.fecha} con {caluroso.temp_max:.1f}°C")
        print(f"Temperatura máxima promedio: {promedio:.1f}°C")

        print("Cantidad de días por cada tipo:")
        tipos = ['Seco', 'Lluvia ligera', 'Lluvia moderada', 'Lluvia intensa']
        for t in tipos:
            cantidad = len(self.dias_por_tipo(t))
            print(f"  - {t}: {cantidad} días")